...
docname: draft-coetzee-oauth-spt-txn-tokens-06
category: exp
ipr: trust200902
area: Security
...
a byproduct of enforcement. Revision -05 added normative specifications for
...
cryptographic algorithm agility including hybrid post-quantum signing. This
revision (-06) hardens attested issuance in response to an adversarial security
...
was minted on. The issuer-side requested-versus-permitted scope intersection of
-05 is retained; the enforcement point re-checks the chain independently.
...
verification model, and jurisdictional token-bound access control. Version -05
added normative Sections {{intent-binding}} (intent binding),
...
implementation. This version (-06) revises {{attested-issuance}} to make the
...
attestation. The issuer's requested-versus-permitted scope intersection from
-05 is reaffirmed, with the enforcement point re-checking the chain
independently ({{delegation}}). The changes reflect an adversarial "find the
